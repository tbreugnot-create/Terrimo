...
import DiagnostiqueurPageClient from './DiagnostiqueurPageClient';
...
async function fetchActeur(slug: string): Promise<ActeurDetail | null> {
  try {
    const rows = await sql`
      ...
      FROM acteurs
      WHERE slug = ${slug} AND type = 'diagnostiqueur' AND is_active = true
      LIMIT 1
    `;
...
export async function generateMetadata(
  ...
  if (!acteur) return { title: 'Diagnostiqueur introuvable — Terrimo' };

  const commune = acteur.commune ?? "Bassin d'Arcachon";
  const desc = acteur.meta?.description
    ?? `${acteur.name} — Diagnostiqueur immobilier certifié à ${commune}. DPE, amiante, électricité, plomb, loi Carrez sur le Bassin d'Arcachon.`;

  return {
    title: `${acteur.name} — Diagnostiqueur immobilier à ${commune} | Terrimo`,
    description: desc.slice(0, 160),
    alternates: { canonical: `https://terrimo.homes/diagnostiqueur/${slug}` },
    openGraph: {
      title: `${acteur.name} — Diagnostiqueur immobilier`,
      description: desc.slice(0, 160),
      type: 'website',
      locale: 'fr_FR',
      siteName: "Terrimo — Bassin d'Arcachon",
      url: `https://terrimo.homes/diagnostiqueur/${slug}`,
    },
    twitter: {
      card: 'summary',
      title: `${acteur.name} — Diagnostiqueur | Terrimo`,
      description: desc.slice(0, 160),
    },
  };
}
...
// ─── Page ─────────────────────────────────────────────────
export default async function DiagnostiqueurPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const acteur = await fetchActeur(slug);
  if (!acteur) notFound();
  ...
  const jsonLd = {
    ...
    '@type': 'LocalBusiness',
    '@id': `https://terrimo.homes/diagnostiqueur/${slug}`,
    url: `https://terrimo.homes/diagnostiqueur/${slug}`,
    name: acteur.name,
    description: acteur.meta?.description ?? `Diagnostiqueur immobilier à ${acteur.commune ?? "Bassin d'Arcachon"}`,
  ...
  return (
    <>
      ...
      <DiagnostiqueurPageClient acteur={acteur} />
    </>
  );
}
